...
COMMENT ON TABLE rt_task_attribute_type IS 'Reference table. Maps EAV attribute types to specific task categories and/or task types.' ;

COMMENT ON COLUMN rt_task_attribute_type.id IS 'Unique ID for an attribute type' ;

COMMENT ON COLUMN rt_task_attribute_type.category_id IS 'The task category to which the attribute type belongs' ;

COMMENT ON COLUMN rt_task_attribute_type.task_type_id IS 'The (optional) task type to which the attribute type belongs. If null then the attribute applies to all tasks in the specified task category' ;

COMMENT ON COLUMN rt_task_attribute_type.eav_attribute_type_id IS 'The attribute type' ;

COMMENT ON COLUMN rt_task_attribute_type.max_allowed IS 'The maximum number of attributes of this type allowed' ;

COMMENT ON COLUMN rt_task_attribute_type.display_order IS 'The order to show the attributes in the UI' ;

COMMENT ON COLUMN rt_task_attribute_type.is_required IS 'Indicates whether or not the attribute is required.' ;
...
ALTER TABLE rt_task_attribute_type
    ...
    REFERENCES rt_eav_attribute_type ( id ) ;
