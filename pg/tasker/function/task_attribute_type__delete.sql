...
CREATE OR REPLACE FUNCTION task_attribute_type__delete (
    a_id integer,
...
BEGIN
    ...
    BEGIN
        ...
        IF user_is_admin ( a_session_username ) THEN
            ...
            DELETE FROM tasker.rt_task_attribute_type
                WHERE id = a_id
                    AND NOT EXISTS (
                        SELECT 1
                            FROM tasker.dt_task_attribute
...
ALTER FUNCTION task_attribute_type__delete (
    integer,
    varchar ) OWNER TO tasker_owner ;

GRANT ALL ON FUNCTION task_attribute_type__delete (
    integer,
    varchar ) TO tasker_user ;

REVOKE ALL ON FUNCTION task_attribute_type__delete (
    integer,
    varchar ) FROM public ;
