...
CREATE OR REPLACE FUNCTION activity_category__insert (
    a_name varchar,
...
BEGIN
    ...
    BEGIN
        ...
        IF user_is_admin ( a_session_username ) THEN
            ...
            WITH a AS (
                INSERT INTO tasker.rt_activity_category (
                        id,
                        category_id,
                        name,
                        description,
                        ...
                        created_by,
                        created_dt )
                    SELECT nextval ( 'rt_activity_category_id_seq' ),
                            a_category_id,
                            trim ( a_name ),
                            trim ( a_description ),
                            a_is_enabled,
                        ...
                        WHERE NOT EXISTS (
                            SELECT 1
                                FROM tasker.rt_activity_category
...
ALTER FUNCTION activity_category__insert (
    varchar,
    varchar,
    boolean,
    varchar ) OWNER TO tasker_owner ;

GRANT ALL ON FUNCTION activity_category__insert (
    varchar,
    varchar,
    boolean,
    varchar ) TO tasker_user ;

REVOKE ALL ON FUNCTION activity_category__insert (
    varchar,
    varchar,
    boolean,
    varchar ) FROM public ;
