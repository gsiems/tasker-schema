...
COMMENT ON COLUMN tasker_data.rt_role.created_by IS 'The ID of the individual that created the row (ref dt_user).' ;

COMMENT ON COLUMN tasker_data.rt_role.created_dt IS 'The timestamp when the row was created.' ;

COMMENT ON COLUMN tasker_data.rt_role.updated_by IS 'The ID of the individual that most recently updated the row (ref dt_user).' ;
