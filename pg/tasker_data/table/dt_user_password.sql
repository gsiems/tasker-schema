...
COMMENT ON COLUMN tasker_data.dt_user_password.created_by IS 'The ID of the individual that created the row (ref dt_user).' ;

COMMENT ON COLUMN tasker_data.dt_user_password.created_dt IS 'The timestamp when the row was created.' ;

COMMENT ON COLUMN tasker_data.dt_user_password.updated_by IS 'The ID of the individual that most recently updated the row (ref dt_user).' ;
