...
CREATE OR REPLACE FUNCTION activity_status__delete (
    a_id integer,
...
BEGIN
    ...
    BEGIN
        ...
        IF user_is_admin ( a_session_username ) THEN
            ...
            DELETE FROM tasker.rt_activity_status
                WHERE id = a_id
                    AND NOT EXISTS (
                        SELECT 1
...
ALTER FUNCTION activity_status__delete (
    integer,
    varchar ) OWNER TO tasker_owner ;

GRANT ALL ON FUNCTION activity_status__delete (
    integer,
    varchar ) TO tasker_user ;

REVOKE ALL ON FUNCTION activity_status__delete (
    integer,
    varchar ) FROM public ;
